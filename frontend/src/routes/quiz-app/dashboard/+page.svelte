  ...
  onMount(async () => {
    ...
    try {
      const [catRes, quizRes, resRes, userRes] = await Promise.all([
        ...
        fetch(`/api/kuisapp/me?t=${Date.now()}`, {
          credentials: "include",
          cache: "no-store",
        }),
      ]);
...
{#if isLoading}
  {#if showLoading}
    <div
      ...
      class="w-full min-h-[50vh] flex flex-col items-center justify-center gap-4"
    >
      <div
        class="w-10 h-10 border-4 border-slate-200 border-t-indigo-600 rounded-full animate-spin"
      ></div>
      <div class="text-sm font-medium text-slate-500 animate-pulse">
        Memuat data kuis...
      </div>
    </div>
  {/if}
{:else}
  <div in:fade={{ duration: 300 }} class="space-y-8 pb-12">
    ...
    >
      <!-- Background element -->
      <div
        class="absolute top-0 right-0 -mt-8 -mr-8 text-white/10 hidden sm:block"
      >
        <svg
          class="w-32 h-32 transform rotate-12"
          fill="currentColor"
      ...
      <div class="relative z-10 flex items-center justify-between">
        <div class="flex items-center">
          <span
            class="font-black tracking-wider uppercase text-md sm:text-lg drop-shadow-sm"
            >LB Quiz</span
          >
        </div>
      </div>
      ...
      <div
        class="relative z-10 flex flex-row justify-between items-center gap-3"
      >
        <!-- Profile Info -->
        <div class="flex flex-row items-center gap-3 sm:gap-4 overflow-hidden">
          ...
          <div class="min-w-0">
            <h1
              class="text-lg sm:text-xl font-bold mb-0.5 tracking-tight truncate"
            >
              {userProfile?.username || "Peserta"}
            </h1>
            <div class="flex items-center gap-2">
              <span
                class="inline-flex items-center px-2 py-0.5 rounded text-[10px] sm:text-xs font-medium bg-white/20 backdrop-blur-sm text-indigo-50 border border-white/10 capitalize"
              >
        ...
        {#if userProfile}
          <div class="flex flex-col items-center shrink-0">
            <div
              class="text-[9px] sm:text-[10px] text-indigo-100/90 font-medium uppercase tracking-widest mb-1"
            >
              Total Poin
            </div>
            <div
              class="flex items-center gap-1.5 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/20 shadow-sm"
            >
              <span
                class="text-xl sm:text-2xl font-bold text-amber-300 drop-shadow-sm"
                >{userProfile.points || 0}</span
              >
            </div>
          </div>
        {/if}
...
>
  <div class="p-2">
    ...
    {#if passwordErrorMsg}
      <div
        class="mb-4 bg-red-50 text-red-600 px-4 py-3 rounded-xl text-sm font-medium border border-red-100 flex items-start gap-2"
      >
        <svg
    ...
    {#if passwordSuccessMsg}
      <div
        class="mb-4 bg-emerald-50 text-emerald-600 px-4 py-3 rounded-xl text-sm font-medium border border-emerald-100 flex items-start gap-2"
      >
        <svg
          class="w-5 h-5 shrink-0 mt-0.5"
          fill="currentColor"
    ...
    <form class="space-y-4" onsubmit={handleChangePassword}>
      <div>
        ...
        <div class="relative">
          <input
            ...
            class="block w-full rounded-xl border border-slate-200 px-4 py-2.5 pr-12 text-sm focus:border-indigo-500 focus:ring-indigo-500 transition-colors"
      ...
      <div class="pt-2 flex justify-end gap-3">
        ...
        <button
          type="submit"
          disabled={isChangingPassword || passwordSuccessMsg !== ""}
          class="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm hover:shadow-indigo-500/25 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2"
        >
