  ...
  <div class="bg-white py-8 px-4 shadow-xl shadow-indigo-100/50 sm:rounded-2xl sm:px-10 border border-slate-100">
    <div class="sm:mx-auto sm:w-full sm:max-w-md mb-8">
      ...
      <p class="mt-2 text-center text-sm text-slate-500">
        Atau <a href="/quiz-app/register" class="font-medium text-indigo-600 hover:text-indigo-500">daftar akun baru</a>
      </p>
    </div>
    ...
    <form class="space-y-6" onsubmit={handleLogin}>
      <div>
        ...
        <div class="mt-2">
          <input id="username" type="text" required bind:value={username} class="block w-full appearance-none rounded-xl border border-slate-200 px-4 py-3 placeholder-slate-400 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm transition-all" placeholder="Masukkan username">
        </div>
      </div>
      ...
      <div>
        ...
        <div class="mt-2 relative">
          <input id="password" type={showPassword ? "text" : "password"} required bind:value={password} class="block w-full appearance-none rounded-xl border border-slate-200 pl-4 pr-10 py-3 placeholder-slate-400 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm transition-all" placeholder="••••••••">
          <button type="button" onclick={() => showPassword = !showPassword} class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 focus:outline-none" title={showPassword ? "Sembunyikan password" : "Tampilkan password"}>
            {#if showPassword}
