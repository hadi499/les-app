...
<div class="max-w-4xl mx-auto md:p-6 lg:p-8 animate-in fade-in duration-500">
  <button
    onclick={goBack}
    ...
    <div class="p-2 md:p-10 transition-all">
      ...
      <div class="flex flex-col gap-3 min-h-[400px]">
        {#if list.items.length === 0}
          <div
            class="p-16 text-center text-slate-500 flex flex-col items-center justify-center h-full opacity-70"
          >
            <svg
              class="w-20 h-20 mb-6 text-slate-300"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              ><path
              ...
              ></path></svg
            >
            <p class="font-medium text-xl text-slate-600 mb-2">
              Belum ada tugas.
            </p>
            <p class="text-base">
              Ketik di atas lalu tekan enter untuk menambah tugas ke list ini.
            </p>
          </div>
        {:else}
          {#each list.items as item (item.id)}
            <div
              ...
              in:slide|local
              out:slide|local
              class="group relative flex items-center justify-between p-4 md:p-5 bg-white border {item.completed
                ? 'border-slate-100 bg-slate-50'
                : 'border-slate-200 shadow-sm'} rounded-2xl transition-all hover:border-blue-300 hover:shadow-md {openMenuId ===
              item.id
                ? 'z-50'
                : 'z-0'}"
            >
              <div
                role="button"
                tabindex="0"
                class="flex items-center gap-3 flex-1 cursor-pointer outline-none"
