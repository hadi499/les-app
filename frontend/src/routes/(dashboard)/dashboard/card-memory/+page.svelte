...
<div class="min-h-screen bg-transparent text-white">
  <header class="fixed top-16 md:top-0 left-0 md:left-64 right-0 bg-zinc-900/90 backdrop-blur-md border-b border-indigo-400/30 px-6 md:px-10 py-3 z-40 transition-all">
    <div
      ...
      <div class="flex items-center gap-4">
        <nav class="flex items-center gap-1 bg-zinc-800/80 p-1 rounded-xl">
          ...
          >
            ...
            {#if queueCount > 0}
              <span
                class="bg-indigo-600 text-white text-[10px] rounded-full px-1.5 py-0.5 leading-none"
                >{queueCount}</span
              >
            {/if}
          </a>
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <div class="relative">
          <input
            ...
            class="px-4 py-1.5 pr-8 text-sm bg-transparent text-white border border-zinc-800 rounded-xl focus:ring-2 focus:ring-gray-200 focus:bg-zinc-900 focus:border-transparent outline-none w-56 transition-all"
            onkeydown={(e) => {
          ...
          {#if searchQuery}
            <button
              ...
              class="absolute right-8 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-blue-200 cursor-pointer text-base"
              >&times;</button
            >
          {/if}
          <button
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex items-center gap-1.5 px-4 py-1.5 text-sm rounded-xl bg-zinc-900 border border-zinc-800 text-blue-100 hover:bg-transparent text-white font-medium transition-all shadow-sm cursor-pointer"
          >
            <svg
              class="w-4 h-4"
              fill="none"
  ...
  <main class="max-w-5xl mx-auto p-4 pt-24 md:pt-20">
    ...
    {#if cards.length > 0 && isTeacher}
      <div class="flex items-center gap-3 mb-3">
        <button
          onclick={selectAll}
          class="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-blue-100 cursor-pointer"
        >
          <span
        ...
        {#if selectedCount > 0}
          ...
          <span class="text-xs text-blue-200">{selectedCount} dipilih</span>
          <button
            onclick={() => (showBulkDeleteConfirm = true)}
            class="px-3 py-1 text-xs rounded-lg bg-red-600 text-white hover:bg-red-700 cursor-pointer"
          >
            Hapus ({selectedCount})
          </button>
        {/if}
    ...
    <div>
      <h2 class="text-md font-semibold text-blue-100 mb-3">
        Arsip ({total} kartu)
        {#if totalPages > 1}
          <span class="text-zinc-500"> · hlm {currentPage}/{totalPages}</span>
        {/if}
      </h2>

      {#if loading}
        <div
          class="text-center py-8 bg-zinc-900 rounded-xl border border-zinc-800"
        >
          <p class="text-zinc-500">Memuat data dari server...</p>
        </div>
      {:else if error}
        <div class="text-center py-8 bg-zinc-900 rounded-xl border border-red-800/50">
      ...
      {:else}
        <div class="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
          <table class="w-full text-sm">
            <thead>
              <tr
                class="bg-transparent text-white border-b border-zinc-800 text-left text-xs text-zinc-400 uppercase tracking-wider"
              >
            ...
            <tbody>
              {#each cards as card (card.id)}
                <tr
                  class="border-b border-zinc-800/50 hover:bg-transparent text-white transition-colors"
                >
                  {#if isTeacher}
                  <td class="px-4 py-3">
                    <button
                  ...
                  <td class="px-4 py-3 font-medium text-white"
                    >{card.title}</td
                  >
                  <td class="px-4 py-3 text-blue-200">{card.category || "-"}</td
                  >
                  <td class="px-4 py-3">
                    <span
                    ...
                    <div class="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        class="p-1.5 rounded-lg cursor-pointer {printQueue.has(
                          ...
                          ? 'bg-indigo-900/50 text-indigo-600'
                      ...
                      <button
                        ...
                        class="p-1.5 rounded-lg text-red-400 hover:text-red-300 hover:bg-red-900/30 cursor-pointer"
                        title="Hapus"
                      >
                        <svg
                          class="w-4 h-4"
        ...
        {#if totalPages > 1}
          <div class="flex items-center justify-center gap-1 mt-6">
            <button
              ...
              class="px-2 py-1 text-xs rounded border border-zinc-700 text-zinc-400 hover:bg-zinc-800 disabled:opacity-30 disabled:cursor-default cursor-pointer"
              >&laquo;</button
            >
            <button
              ...
              {#if p === 1 || p === totalPages || (p >= currentPage - 2 && p <= currentPage + 2)}
                <button
                  ...
                  currentPage
                    ...
                    : 'border-zinc-700 text-blue-200 hover:bg-zinc-800'}"
                  >{p}</button
                >
  ...
  <Modal show={!!deleteCardId} onclose={() => (deleteCardId = null)}>
    <div class="space-y-4 text-center">
      ...
      <div>
        <h3 class="text-lg font-semibold text-white">Hapus Kartu</h3>
        {#if deleteTarget}
  ...
  <Modal show={!!detailCard} onclose={() => { detailCard = null; cardWrapperWidth = 0; }}>
    {#if detailCard}
      <div class="space-y-5 w-full {getModalMaxWidth(detailCard.size)} mx-auto pb-2 transition-all duration-300">
        <div 
          class="w-full mx-auto shadow-lg rounded-xl overflow-hidden border border-zinc-800 transition-all duration-300 bg-zinc-900"
          style="aspect-ratio: {getCardAspect(detailCard.size)};"
          bind:clientWidth={cardWrapperWidth}
        >
          {#if cardWrapperWidth > 0}
            <div style="width: {getCardPrintWidth(detailCard.size)}mm; height: {getCardPrintHeight(detailCard.size)}mm; transform: scale({cardWrapperWidth / (getCardPrintWidth(detailCard.size) * 3.7795)}); transform-origin: top left;">
        ...
        <div class="flex items-center justify-center gap-4">
          <button
            ...
            class="w-12 h-12 flex items-center justify-center rounded-full transition-all cursor-pointer shadow-sm border {printQueue.has(detailCard.id) ? 'bg-indigo-900/30 border-indigo-800/50 text-indigo-600 hover:bg-indigo-900/50 shadow-indigo-100' : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-indigo-600 hover:border-indigo-800/50 hover:bg-indigo-900/30'}"
            title={printQueue.has(detailCard.id) ? "Hapus dari Print" : "Tambah ke Print"}
          >
          ...
          <button
            ...
            class="w-12 h-12 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-amber-600 hover:border-amber-300 hover:bg-amber-50 transition-all cursor-pointer shadow-sm"
            title="Edit Kartu"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="w-12 h-12 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-red-400 hover:border-red-300 hover:bg-red-900/30 transition-all cursor-pointer shadow-sm"
            title="Hapus Kartu"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="w-12 h-12 flex items-center justify-center rounded-full bg-gray-800 border border-gray-800 text-white hover:bg-indigo-600 hover:border-gray-900 transition-all cursor-pointer shadow-md hover:shadow-lg"
            title="Tutup"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
