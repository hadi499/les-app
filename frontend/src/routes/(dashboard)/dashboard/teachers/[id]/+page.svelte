...
<svelte:head>
  <title>Detail Guru - Portal Admin</title>
</svelte:head>
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
      <div class="flex items-center gap-2 mb-2">
        <a href="/dashboard/teachers" class="text-blue-600 hover:text-blue-800 flex items-center gap-1 text-sm font-medium transition-colors">
      ...
      <p class="mt-2 text-slate-600 text-sm sm:text-base font-light tracking-wide">
        Informasi detail mengenai akun guru ini.
      </p>
    </div>
  </div>
      ...
      <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            Daftar Murid yang Diajar
          </h3>
          <span class="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">{user.students ? user.students.length : 0} Murid</span>
        </div>
            ...
            {#each user.students as student}
              <div class="flex items-center gap-3 bg-white p-4 rounded-2xl shadow-sm border border-slate-100 hover:border-blue-200 transition-colors">
                ...
                <div class="flex-1 min-w-0">
                  <p class="font-bold text-slate-900 truncate">{student.username}</p>
        ...
        {:else}
          <div class="text-center py-8 bg-white rounded-2xl border border-slate-100 border-dashed">
            ...
            <p class="text-slate-500 font-medium">Belum ada murid yang diajar.</p>
          </div>
        {/if}
      </div>
