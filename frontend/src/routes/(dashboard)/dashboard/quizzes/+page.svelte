...
<div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
  ...
  {#if isTeacher && activeTab === "quizzes"}
    <a
      ...
      class="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg font-medium hover:bg-orange-700 transition-colors shadow-sm no-underline"
    >
...
<div class="flex gap-2 mb-6 border-b border-orange-200">
  ...
  <button
    class="px-6 py-3 font-semibold text-sm border-b-2 transition-colors cursor-pointer {activeTab === 'scores' ? 'border-orange-600 text-orange-900' : 'border-transparent text-orange-700 hover:text-orange-900'}"
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-orange-50/50 border-b border-orange-200">
              ...
              <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Judul Kuis</th>
              <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Kategori</th>
              <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Batas Waktu</th>
              <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider text-right">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-orange-100">
            {#each quizzes as quiz}
              <tr class="hover:bg-orange-50/50 transition-colors">
                <td class="px-6 py-4 text-sm font-medium text-orange-950">#{quiz.id}</td>
                <td class="px-6 py-4 text-sm font-semibold text-orange-900">{quiz.title}</td>
                <td class="px-6 py-4 text-sm text-orange-800">
                  <span class="px-2 py-1 bg-orange-100 text-orange-800 rounded-md text-xs font-medium border border-orange-200">
                    {quiz.category}
                  </span>
                </td>
                <td class="px-6 py-4 text-sm text-orange-800">{quiz.timeLimit} Menit</td>
                <td class="px-6 py-4 text-right space-x-2">
                  ...
                  {:else}
                    <a
                      href="/quiz/{quiz.id}"
                      class="inline-flex items-center justify-center px-4 py-2 text-xs font-bold text-white bg-orange-500 rounded-lg hover:bg-orange-600 transition-colors no-underline"
                    >
                      Kerjakan Kuis
                    </a>
                  {/if}
            ...
            {:else}
              <tr>
                <td colspan="5" class="px-6 py-8 text-center text-sm text-orange-800">
  ...
  {#if activeTab === "scores"}
    {#if isLoadingScores}
      <div class="flex justify-center p-8">
        <div class="w-6 h-6 border-4 border-orange-200 border-t-orange-600 rounded-full animate-spin"></div>
      </div>
    {:else}
      <div class="bg-white rounded-2xl shadow-sm border border-orange-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-orange-50/50 border-b border-orange-200">
                ...
                {#if isTeacher}
                  <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Siswa</th>
                {/if}
                <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Skor</th>
                <th class="px-6 py-4 text-xs font-bold text-orange-900 uppercase tracking-wider">Waktu Selesai</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-orange-100">
              {#each scores as s}
                <tr class="hover:bg-orange-50/50 transition-colors">
                  <td class="px-6 py-4 text-sm font-semibold text-orange-900">
                    {s.quiz ? s.quiz.title : `Kuis #${s.quiz_id}`}
                  </td>
                  {#if isTeacher}
                    <td class="px-6 py-4 text-sm text-orange-800 font-medium">@{s.username}</td>
                  {/if}
                  <td class="px-6 py-4">
              ...
              {:else}
                <tr>
                  <td colspan={isTeacher ? 4 : 3} class="px-6 py-8 text-center text-sm text-orange-800">
